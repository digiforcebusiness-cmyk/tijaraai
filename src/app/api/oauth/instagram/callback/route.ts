// GET /api/oauth/instagram/callback?code=xxx&state=xxx
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (!decoded || decoded.platform !== "INSTAGRAM")
  ...
  if (!tokenRes.ok) {
    console.error("[Instagram OAuth]", await tokenRes.text());
  ...
  await prisma.integration.upsert({
    where: { userId_type: { userId: decoded.userId, type: "INSTAGRAM" } },
    create: { userId: decoded.userId, type: "INSTAGRAM", accessToken: finalToken, isActive: true },
    update: { accessToken: finalToken, isActive: true, lastSyncAt: new Date() },
  });

  return NextResponse.redirect(new URL("/integrations?success=INSTAGRAM", appUrl()));
}
